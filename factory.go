...
func (f *eventhubReceiverFactory) createLogsReceiver(
	...
	r, err := f.getReceiver(pipeline.SignalLogs, cfg, settings)
	if err != nil {
		return nil, err
	}
	r.(dataConsumer).setNextLogsConsumer(nextConsumer)
	return r, nil
}
...
func (f *eventhubReceiverFactory) createMetricsReceiver(
	...
	r, err := f.getReceiver(pipeline.SignalMetrics, cfg, settings)
	if err != nil {
		return nil, err
	}
	r.(dataConsumer).setNextMetricsConsumer(nextConsumer)
	return r, nil
}
...
func (f *eventhubReceiverFactory) createTracesReceiver(
	...
	r, err := f.getReceiver(pipeline.SignalTraces, cfg, settings)
	if err != nil {
		return nil, err
	}
	r.(dataConsumer).setNextTracesConsumer(nextConsumer)
	return r, nil
}
