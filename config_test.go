...
func TestLoadConfig(t *testing.T) {
	...
	tests := []struct {
		...
		{
			id: component.NewIDWithName(metadata.Type, "kafka_auth"),
			expected: &Config{
				Protocol: ProtocolKafka,
				EventHub: EventHubConfig{
					Name:      "hubName",
		...
		{
			id:                  component.NewIDWithName(metadata.Type, "kafka_invalid_protocol"),
			expectedErrContains: "unsupported protocol",
		},
		{
			id:                  component.NewIDWithName(metadata.Type, "kafka_with_blob_checkpoint_store"),
			expectedErrContains: "blob_checkpoint_store is not supported with the Kafka protocol",
		},
		{
			id:                  component.NewIDWithName(metadata.Type, "kafka_with_partition"),
			expectedErrContains: "partition and offset are not supported with the Kafka protocol",
		},
	}
