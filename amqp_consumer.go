...
// UnmarshalJSON handles the legacy "seqNumber" field name for backwards
// compatibility with checkpoints written by older versions of the receiver.
func (c *checkpointSeqNumber) UnmarshalJSON(data []byte) error {
	var tmp struct {
		SequenceNumber int64  `json:"sequenceNumber"`
		SeqNumber      *int64 `json:"seqNumber"` // legacy field name
	}
	if err := json.Unmarshal(data, &tmp); err != nil {
		return err
	}
	c.SequenceNumber = tmp.SequenceNumber
