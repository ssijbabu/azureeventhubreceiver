...
type eventhubHandler struct {
	...
	consumerClient *azeventhubs.ConsumerClient // non-nil when in AMQP distributed mode
	kafkaCG        sarama.ConsumerGroup        // non-nil when in Kafka mode
	wg             sync.WaitGroup              // tracks goroutines spawned by runDistributed / runKafka
}
...
func (h *eventhubHandler) run(ctx context.Context, host component.Host) error {
	...
	if h.config.Protocol == ProtocolKafka {
		return h.runKafka(ctx, host)
	}
	return h.runAMQP(ctx, host)
}

func (h *eventhubHandler) runAMQP(ctx context.Context, host component.Host) error {
...
func (h *eventhubHandler) runKafka(ctx context.Context, host component.Host) error {
	cg, topic, err := buildKafkaConsumerGroup(h.config, host, h.settings.Logger)
	if err != nil {
		return err
	}
	h.kafkaCG = cg

	kHandler := &kafkaConsumerGroupHandler{
		handler: h.newMessageHandler,
	...
	h.wg.Go(func() {
		for {
			...
			if err := cg.Consume(ctx, []string{topic}, kHandler); err != nil && ctx.Err() == nil {
				h.settings.Logger.Error("kafka consumer group error", zap.Error(err))
			}
		}
	})
...
func (h *eventhubHandler) close(ctx context.Context) error {
	...
	if h.hub != nil {
		if err := h.hub.Close(ctx); err != nil {
			errs = errors.Join(errs, err)
		}
		h.hub = nil
	}
	...
	// Wait for goroutines spawned by runDistributed / runKafka to finish
	// before closing the clients they depend on.
	h.wg.Wait()
	...
	if h.kafkaCG != nil {
		if err := h.kafkaCG.Close(); err != nil {
			errs = errors.Join(errs, err)
		}
		h.kafkaCG = nil
	}
